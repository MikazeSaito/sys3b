...
if all_walk_data: # データが1つでも存在する場合にのみグラフを生成
    ...
    # label: 凡例に表示する名前
    if walk1:
        sns.histplot(walk1, bins=bins, kde=True, color='skyblue', edgecolor='black', alpha=0.6, label='System 1 (New)')
    
    # System 2 (従来のシステム) のヒストグラムを同じグラフにプロット
    if walk2:
        sns.histplot(walk2, bins=bins, kde=True, color='lightcoral', edgecolor='black', alpha=0.6, label='System 2 (Old)')
