...
def make_human_waiting_time( human , road_width): #人の待ち時間計算
    def human_waiting_time( locate , start_time , end_time ):
        x = human[ 0 ][ 0 ]
        want =human[3]
        
        x_speed = human[ 1 ][ 0 ]
        w=np.exp(0.001*(abs(x_speed)*(want-x)/x_speed+200))**2
        if ( locate - x ) / x_speed < 0:
            ...
            return (start_time - ( locate - x ) / x_speed)*w
        elif locate < start_time * x_speed + x and locate - road_width / 2> end_time * x_speed + x:
            return 0
        else:
            return 1500*w
...
def calculate2( humans1above,humans1below , cars , a , b ,cross_width , road_width,virtual_time): #横断歩道の最適化
    ...
    for human in humans:
        if a - virtual_time*human[1][0]-30 < human[ 0 ][ 0 ] < b - virtual_time*human[1][0]+ 30:
            func_list.append( make_human_waiting_time( human , road_width) )
    for car in cars:
    ...
    for locate in range( a-40 , b+40 , 10 ):
        for start_time in range( 100 , 1000 , 30):
            for end_time in range( start_time , 1500, 30 ):
                sum = 0
                for func in func_list:
...
def calculate4(humans1below , humans1above ,virtual_time):
    ...
    group = group_numbers_greedy_min_groups( future_human_list , 150)
    for L in group:
